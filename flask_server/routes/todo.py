...
def get_todo_by_id(list_id, todo_id):
    ...
    got_todo = todos.get_todo_by_id(user, list_id, todo_id)
    if not got_todo:
...
def remove_todo(list_id, todo_id):
    ...
    if user is None:
        return jsonify({'error' : 'user not found'}), 401
    was_deleted = todos.remove(user, list_id, todo_id)
...
def edit_todo(list_id, todo_id):
    ...
    was_edited = todos.edit(user, list_id, todo_id, data["new_name"])
...
@todo_bp.route("/<list_id>/<todo_id>/description", methods=["PATCH"])
def edit_description(list_id, todo_id):
    data = request.json
    jwtoken = request.headers.get("Authorization")
    user = get_user(jwtoken)
    if user is None:
        return jsonify({'error' : 'user not found'}), 401
    if "new_description" not in data:
        return jsonify({ "error" : "Bad request. JSON body needs 'new_description'"}), 400
    
    was_edited = todos.edit_description(user, list_id, todo_id, data["new_description"])
...
def update_status_todo(list_id, todo_id):
    ...
    was_updated = todos.update_status(user, list_id, todo_id, data["status"])
